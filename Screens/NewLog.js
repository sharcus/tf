...
import { getDateString, getTimeString } from '../BusinessLogic/CalendarHelper';
  ...
  <ScrollView>
    ...
    <View style={styles.row}>
            {isDatePickerVisible && <DateTimePicker
                ...
                /> }
                <Text style={styles.dateLabel} onPress={showDatePicker}>{getDateString(date)}</Text>
            </View>
            <View style={styles.row}>
                <Text>Activity</Text>
            </View>
            ...
            <View style={styles.row}>
                {isFromPickerVisible && <DateTimePicker 
                    value={from}
                    mode={"time"}
                    is24Hour={true}
                    display="default"
                    ...
                    />}
                    <Text style={styles.twinControl} onPress={showFromPicker}>{getTimeString(from)}</Text>

                    {isToPickerVisible && <DateTimePicker 
                    value={to}
                    mode={"time"}
                    is24Hour={true}
                    display="default"
                    onChange={setToField}
                    />}
                    <Text style={styles.twinControl} onPress={showToPicker}>{getTimeString(to)}</Text>
            </View>
            <View style={styles.row}>
                <Text>Description</Text>
            </View>
            <View style={styles.row}>
                <TextInput style={styles.descriptionControl} value={description} onChangeText={setDescription}></TextInput>
            </View>
            <View style={styles.row}>
            <FeederButton style={styles.buttomButton} Text="Accept" onPress={onAccept}></FeederButton>
            <FeederButton style={styles.buttomButton} Text="Cancel" onPress={onCancel}></FeederButton>
            </View>
    </View>
  </ScrollView>
